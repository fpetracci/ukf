...
%% system definition - RLC
% v_in = R * i + L * di/dt + v_C 

%from which:
% di/dt = - R/L i - v_C / L + v_in /L
% dv_c/dt =  i /C
...
delta_t = 1/1e6; % Sampling period [s]
...
t_end = delta_t*1.5e3;			% duration of simulation [s]
...
s_sigma = 1e-3;		% state standard dev
...
% constant input
u  = u0 * ones(n_in, n_sample);		


% % sinusoidal input
% n_repeats = 3;
% for i = 1:n_sample
% 	u(:,i)  = u0*sin(time(i)/t_end * 2*n_repeats* pi) * ones(n_in, 1);		
% end

% % constant + sinusoidal input
% n_repeats = 20;
% for i = 1:n_sample
% 	u(:,i)  = (u0 + u0/2*sin(time(i)/t_end * 2*n_repeats* pi)) * ones(n_in, 1);		
% end
...
P_init = eye(n_state).*(s_sigma^2);			% initial noise state covariance matrix
